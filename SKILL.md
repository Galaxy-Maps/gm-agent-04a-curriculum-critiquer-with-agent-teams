...
version: 1.0.0
author: Galaxy Maps
repository: https://github.com/Galaxy-Maps/gm-agent-04a-curriculum-critiquer-with-agent-teams
standalone: true
mode: agent-team
teamSize: 4
model: ultra
...
| Tool | Purpose |
|------|---------|
| `Read` | Read INTENT.md, MAP_V{n}.md, and each role's `roles/{role}.md` prompt file |
| `Write` | Write the 4 `critiques/CRITIQUE_*.md` files and the synthesized `MAP_V{n}_SUGGESTIONS.md` |
| `Bash` | `git add critiques/ MAP_V{n}_SUGGESTIONS.md && git commit` |
| `TeamCreate` / `TeamDelete` | Open/close the 4-reviewer Agent Team |
| `Task` | Spawn each reviewer as a teammate from `roles/*.md` |
| `SendMessage` | Reviewers cross-challenge findings during the debate phase |
| `AskUserQuestion` | User approves / declines / modifies each ranked suggestion |

---

## Standalone Usage

When invoked by the orchestrator (`gm-agent-01a-orchestrator-with-agent-teams`), the orchestrator handles the team lifecycle below. When invoked directly (e.g. `Skill({ skill: "gm-agent-04a-curriculum-critiquer-with-agent-teams" })`), this skill runs the team itself:

1. `Read` the 4 role prompts from `roles/pedagogy-reviewer.md`, `roles/learner-advocate.md`, `roles/scope-auditor.md`, `roles/devils-advocate.md`.
2. `TeamCreate({ team_name: "curriculum-critique-team" })`.
3. Spawn each reviewer with `Task({ name, team_name, subagent_type: "general-purpose", prompt: <shared context + persona> })`, where shared context points to `INTENT.md` and `MAP_V{n}.md`.
4. Wait for all 4 `critiques/CRITIQUE_*.md` files to land. Reviewers cross-challenge via `SendMessage`.
5. Synthesize the ranked consensus into `MAP_V{n}_SUGGESTIONS.md` (see Output Format).
6. Walk the user through each suggestion via `AskUserQuestion` (approve / decline / modify).
7. `TeamDelete()` and commit: `git add critiques/ MAP_V{n}_SUGGESTIONS.md && git commit -m "review(curriculum): add 4-reviewer critique for MAP v{n}"`.
